...
# Clean data
We have three data files -- strawberries, insecticides, and herbicides-fungicides-other. The first two data files are used as data basis of our project, and the third one is the backgroud information we need in the project.
...
These two issues are what our team members intend to start studying. Because the main content of the pes file is the toxins of pesticides, we began to study the degree of toxins to humans and to bees.

## show all states chemical usage value
Compared with other states, California has more data, so there are more samples with vlue as high, while Washington state has less data. It can be seen from the figure that the pesticides that are more harmful to humans are mainly distributed in California and Florida.
```{r,message=FALSE}
...
Take California as an example, and list the data graph and plot.
```{r,message=FALSE}
